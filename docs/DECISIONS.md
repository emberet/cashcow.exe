...
## 35. A merged fix is not a live fix: no build step cuts both ways

**Symptom.** §34's own fix — merged, tested, on `main` at 21:29Z — was still
producing zero effect on the running bot a full day later. Checked directly:
`node src/cli.ts feeds --feed dexActivity` (a fresh process) returned 5 real
signals; `signals` had zero `dexActivity` rows in the last 24h from the
long-running one. On closer look the checked-out `main` on the machine
running the bot was itself a commit *behind* `origin/main` — the merge had
happened on GitHub and nothing had pulled it down locally, so even a restart
alone would not have picked it up.

**Why this is worth writing down.** "No build step, Node strips types at
import" is usually the ergonomics win described at the top of this file. The
same property means a code change has *two* separate preconditions before it
affects behaviour — present on disk, and the process re-imports it — and
nothing in this repo checks either one automatically. This is the second time
in as many days this exact shape of bug has cost real signal: the
`ingestSignals()` dedup fix (stopping re-served feed content from faking
fresh trend activity) was merged mid-run on 2026-08-27 and sat inert for
~4 hours in the then-running process, inflating `signals` the whole time,
until an unrelated SIGKILL-and-relaunch happened to restart it and the fix
